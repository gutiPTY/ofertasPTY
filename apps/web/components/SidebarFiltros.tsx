...
export default function SidebarFiltros({ categorias }: { categorias: Categoria[] }) {
  ...
  const qActiva = searchParams.get("q") ?? "";

  const hayFiltrosActivos = Boolean(
    categoriaActiva || provinciaActiva || precioMinActivo || precioMaxActivo || qActiva,
  );
    ...
    <aside className="flex flex-col gap-6 md:w-full">
      ...
      <div className={`${abierto ? "flex" : "hidden"} flex-col gap-6 md:flex`}>
        <form
          ...
          className="flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2 text-sm text-muted"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" aria-hidden="true" className="shrink-0">
